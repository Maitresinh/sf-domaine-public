...
cur.execute('''
    SELECT
        ...
        GROUP_CONCAT(DISTINCT CASE WHEN a.author_language = 22 THEN a.author_canonical END SEPARATOR " / ") AS traducteurs
    FROM titles t
...
for r in cur.fetchall():
    ...
    if oid not in vf_map or (r['last_vf_year'] and (not vf_map[oid]['last_vf_year'] or r['last_vf_year'] > vf_map[oid]['last_vf_year'])):
        # Préserver les traducteurs déjà trouvés si le nouveau n'en a pas
        prev_trad = vf_map[oid]['traducteurs'] if oid in vf_map else None
        vf_map[oid] = r
        if not r['traducteurs'] and prev_trad:
            vf_map[oid]['traducteurs'] = prev_trad
    else:
        # Accumuler traducteurs des autres titres FR du même original
        if r['traducteurs'] and not vf_map[oid]['traducteurs']:
            vf_map[oid]['traducteurs'] = r['traducteurs']
        elif r['traducteurs'] and vf_map[oid]['traducteurs'] and r['traducteurs'] not in vf_map[oid]['traducteurs']:
            vf_map[oid]['traducteurs'] += ' / ' + r['traducteurs']
